...
const int noiseSensorPin = A0;     // soundsensor
const int threshold = 110; // 임계값 설정
...
void loop() {
  ...
  //db측정값이 기준 이상이면 led켜지게
  if (sensorValue > thresholdValue) {
    Serial.println("Threshold exceeded. Turning on the LED!");
    ledState = HIGH;
  } else {
    ledState = LOW;
  }
  ...
  //led 제어
  digitalWrite(ledPin, ledState); // LED 상태 갱신
...
void connectMQTT() {
  ...
  mqttClient.subscribe("$aws/things/sangwon/shadow/update/delta");
}

void getDeviceStatus(char* payload) {
  // Read temperature as Celsius (the default)
  int t = analogRead(noiseSensorPin);

  // Read led status
  const char* led = (ledState == HIGH)? "ON" : "OFF";

  // make payload for the device update topic ($aws/things/MyMKRWiFi1010/shadow/update)
  sprintf(payload,"{\"state\":{\"reported\":{\"noiseDb\":\"%0.2f\",\"LED\":\"%s\"}}}",t,ledState);
}

void sendMessage(char* payload) {
  char TOPIC_NAME[]= "$aws/things/sangwon/shadow/update";
